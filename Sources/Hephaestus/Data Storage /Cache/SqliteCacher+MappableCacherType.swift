...
extension SqliteCacher: MappableCacherType {
    ...
    func saveObject<T: Codable>(_ object: T) {
        if let jsonString = ParseWorker.parse(object) {
    ...
    func getObject<T: Codable>(of type: T.Type) -> T? {
        if let stringValue = getString(forMappableName: String(describing: T.self)),
           let object: T = ParseWorker.parse(stringValue) {
            return object
        }
        return nil
    }
    ...
    func saveArray<T: Codable>(_ array: [T]) {
        if let jsonString = ParseWorker.parse(array) {
            saveString(jsonString, forMappableName: String(describing: T.self))
        }
    }
    ...
    func getArray<T: Codable>(of type: T.Type, withId identifier: String?) -> (array: [T], needReload: Bool) {
        ...
        do {
            ...
            for result in try connection.prepare(select) {
                ...
                if let array: [T] = ParseWorker.parse(stringValue){
    ...
    func getArray<T>(of type: T.Type) -> (array: [T], needReload: Bool) where T : Codable {
